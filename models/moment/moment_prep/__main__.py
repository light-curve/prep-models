"""Entry point: uv run python -m moment_prep <command> --size <size> [opts]

Like the Chronos-Bolt package, MOMENT ships several sizes from one package, so
every command takes a required ``--size`` (small/base/large) and reads/writes a
per-size ``out/<size>/`` subdirectory.
"""
...
from moment_prep.config import DEFAULT_SIZE, SIZES, out_dir


def main() -> None:
    parser = argparse.ArgumentParser(prog="moment_prep")
    ...
    def _add_size(p: argparse.ArgumentParser) -> None:
        p.add_argument(
            ...
            help="MOMENT-1 size to operate on.",
        )
    ...
    if args.command == "fetch":
        import_module("moment_prep.fetch").run_fetch(update=args.update, size=size)
    elif args.command == "download":
        import_module("moment_prep.download").run_download(force=args.force, size=size)
    elif args.command == "export":
        output_dir = args.output_dir or out_dir(size) / "onnx"
        import_module("moment_prep.export").run_export(output_dir, size)
    elif args.command == "test-data":
        output_dir = args.output_dir or out_dir(size) / "test-data"
        import_module("moment_prep.test_data").run_test_data(
