...
def run_export(out_dir: Path, checkpoint_dir: Path, device: str = "cpu") -> None:
    ...
    _write_glue_readme(out_dir, hidden_dim, N_points, encoder.t_max, encoder.NUM_BANDS)


def _write_glue_readme(
    out_dir: Path, hidden_dim: int, N_points: int, t_max: float, num_bands: int
) -> None:
    text = f"""# SELDON ONNX inference glue loop

Files in this directory:
  seldon_embed.onnx     inputs: flux(B,1), band_idx(B,), time(B,)
                        outputs: x_embedded(B,{hidden_dim}), scaled_time(B,)
  seldon_ode_step.onnx  inputs: h(B,{hidden_dim}), t0(B,), t1(B,)
                        outputs: h_next(B,{hidden_dim})
  seldon_gru_cell.onnx  inputs: x_embedded(B,{hidden_dim}), h(B,{hidden_dim}), mask(B,)
                        outputs: h_next(B,{hidden_dim})
  seldon_deepset.onnx   inputs: zs_t(B,{N_points},{hidden_dim + 1})
                        outputs: z(B,{hidden_dim})

Inference pseudocode (seq_len observations, already sorted by time desc):

    DT = 1e-4   # small dt to avoid t0==t1 singularity

    # embed all observations
    x_emb  = []
    t_scaled = []
    for each observation t:
        xe, ts = embed(flux[t], band[t], time[t])
        x_emb.append(xe);  t_scaled.append(ts)

    # RNN-ODE loop (backwards in time)
    h = zeros(B, {hidden_dim})
    for t in range(T-1, 0, -1):
        t0 = t_scaled[t]
        t1 = t_scaled[t-1]
        same = (t1 == t0)
        t1_adj = t1 - DT * same
        h = ode_step(h, t0, t1_adj)
        h = gru_cell(x_emb[t-1], h, mask[t-1])

    # aggregation: evolve h over a fixed time grid
    t_grid = linspace(0, {t_max}, {N_points}) * softplus(time_scaling)
    zs = [h]
    for i in range(1, {N_points}):
        h = ode_step(h, t_grid[i-1], t_grid[i])
        zs.append(h)
    zs = stack(zs)                            # (B, {N_points}, {hidden_dim})
    zs_t = cat(zs, t_grid_expanded, dim=-1)   # (B, {N_points}, {hidden_dim + 1})
    z = deepset(zs_t)                         # (B, {hidden_dim})

Note: time_scaling is a learned scalar parameter.  Its softplus value should
be saved alongside the ONNX files (it equals softplus(encoder.time_scaling)).
NUM_BANDS = {num_bands}
"""
    (out_dir / "INFERENCE.md").write_text(text)
